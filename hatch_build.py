...
`pip install pycontacts` works with no user setup.

So the build copies it in from outside the tree, trying in order:

  1. $PYCONTACTS_CLIENT_SECRET  -- for release automation (CI secret -> file)
  2. ~/.config/pycontacts/client_secret.json  -- the usual developer location
  3. src/pycontacts/client_secret.json  -- already in place, left alone
...
CLIENT_SECRET = os.path.join("src", "pycontacts", "client_secret.json")
CONFIG_LOCATION = os.path.expanduser("~/.config/pycontacts/client_secret.json")
...
class CustomBuildHook(BuildHookInterface):
    def initialize(self, version, build_data):
        ...
        for source in (os.environ.get("PYCONTACTS_CLIENT_SECRET"), CONFIG_LOCATION):
        ...
        raise RuntimeError(
            ...
            f"{CONFIG_LOCATION},\n"
            "or set PYCONTACTS_CLIENT_SECRET to its path, before building."
        )
